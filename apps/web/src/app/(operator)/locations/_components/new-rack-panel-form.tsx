...
interface Props {
  rackId: string;
  redirectTo: string;
  createAction: (rackId: string, dto: CreateRackPanelDto) => Promise<unknown>;
}

export function NewRackPanelForm({ rackId, redirectTo, createAction }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const uPositionRaw = fd.get('uPosition') as string;
    const outcome = await createAction(rackId, {
      name: fd.get('name') as string,
      code: (fd.get('code') as string).toUpperCase(),
      panelType: fd.get('panelType') as string,
      portCount,
      uPosition: uPositionRaw ? parseInt(uPositionRaw, 10) : undefined,
    ...
    >
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-gray-500">
          Rack unit where this panel is mounted (1 = top)
        </p>
      </div>
