...
import { buildingsApi, cagesApi, panelsApi, racksApi, roomsApi, sitesApi } from '@/lib/api/locations';
import { auth } from '@/lib/auth/session';
import { notFound } from 'next/navigation';

export default async function EditRackPanelPage({
  params,
}: {
  params: Promise<{ siteId: string; buildingId: string; roomId: string; cageId: string; rackId: string; panelId: string }>;
}) {
  const { siteId, buildingId, roomId, cageId, rackId, panelId } = await params;
  const session = await auth();
  const token = (session?.user as any)?.accessToken as string;

  const [site, buildings, room, cages, racks, panel] = await Promise.all([
  ...
  if (!site || !building || !room || !cage || !rack || !panel) notFound();
