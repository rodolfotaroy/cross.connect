...
export function NewOpTicketForm({ orgs }: { orgs: { id: string; name: string; code: string }[] }) {
  ...
  async function execute(formData: FormData) {
    ...
    setError('');
    try {
      const ticket = await opSupportApi.createTicket(token, {
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="block text-sm font-medium text-gray-700" htmlFor="orgId">
            Organization *
          </label>
          <select
            id="orgId"
