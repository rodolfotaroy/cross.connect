...
import { CustomerAddCommentForm } from './add-comment-form';
import { CustomerUpdateStatusForm } from './update-status-form';

export const metadata: Metadata = { title: 'Ticket — Customer Portal' };
...
export default async function CustomerTicketDetailPage({
  ...
  const ticket = await customerSupportApi.getTicket(token, id).catch(() => null);
  if (!ticket) notFound();
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6">
        ...
        {ticket.status !== 'closed' && (
          <div className="mt-6 border-t border-gray-100 pt-6">
            ...
            <CustomerAddCommentForm ticketId={id} />
          </div>
        )}
      </div>
    </div>
